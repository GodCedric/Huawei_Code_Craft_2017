...
void generateChorm(Chorm& chorm, vector<double>& probability, int chormNum, int geneBit, int maxServers){
    ...
    //int numservers = rand() % (maxServers-maxServers/3) + maxServers/3;   //随机生成服务器数量，最多maxServers个
...
void crossover(const double crossoverRate,
    ...
    if(cntValidChorm < 10){
        ...
        //前5个最小有效解两两交叉获得10个新的有效解,放在尾部
        int cnt = 0;
        for(int i=0;i<5;i++){
            for(int j=i+1;j<5;j++){
                cnt++;
                int a = chormNum-20-cnt;
                new_population[a] = population[i];
                cnt++;
                int b = chormNum-20-cnt;
